...
import { BloodRequest, Prisma } from "../../generated/prisma/client.js";
...
type BloodRequestCreatePayload = {
  hospitalName: string;
  hospitalAddress: string;
  patientName: string;
  patientAge: number;
  patientBloodGroup: import("../../generated/prisma/enums.js").BloodGroup;
  bloodUnits: number;
  isEmergency: boolean;
  contactNumber: string;
  dateOfDonation: string | Date;
  description?: string;
  requestToDonorId?: string;
  status?: RequestStatus;
};

type BloodRequestUpdatePayload = Partial<BloodRequestCreatePayload>;
...
const getRequesterDonorId = async (currentUser: IRequestUser) => {
...
const getAccessWhere = async (currentUser: IRequestUser): Promise<Prisma.BloodRequestWhereInput> => {
  ...
  return {
    isDeleted: false,
    requestToDonorId: donorId as string,
  };
};

const assertCanManageBloodRequest = async (
  bloodRequest: { userId: string; requestToDonorId: string | null },
...
const getBloodRequests = async (query: IQueryParams, currentUser: IRequestUser) => {
  const where = await getAccessWhere(currentUser);

  const queryBuilder = new QueryBuilder<BloodRequest, Prisma.BloodRequestWhereInput, Prisma.BloodRequestInclude>(
    prisma.bloodRequest,
    query,
    {
...
const getBloodRequestById = async (id: string, currentUser: IRequestUser) => {
...
const createBloodRequest = async (payload: BloodRequestCreatePayload, currentUser: IRequestUser) => {
...
const updateBloodRequest = async (
  id: string,
  payload: BloodRequestUpdatePayload,
  currentUser: IRequestUser,
) => {
  const bloodRequest = await prisma.bloodRequest.findFirst({
    where: {
...
const softDeleteBloodRequest = async (id: string, currentUser: IRequestUser) => {
  const bloodRequest = await prisma.bloodRequest.findFirst({
    where: {
      id,
      isDeleted: false,
