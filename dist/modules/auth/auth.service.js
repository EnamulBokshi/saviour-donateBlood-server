...
const assertAccountUsable = (userStatus, isDeleted) => {
    ...
    if (userStatus === UserStatus.BANNED) {
        throw new AppError(status.FORBIDDEN, "Your account is currently restricted. Please contact support.");
    }
    if (userStatus === UserStatus.INACTIVE) {
        throw new AppError(status.FORBIDDEN, "Your account is currently inactive. Please verify your email or contact support.");
    }
};
const normalizeEmail = (email) => email.trim().toLowerCase();
const ensureDonorProfilePayload = (donorProfile) => {
    if (!donorProfile) {
        throw new AppError(status.BAD_REQUEST, "Donor profile details are required when you sign up as a donor.");
    }
    return donorProfile;
};
const uploadImageWithTimeout = async (file, timeoutMs = 10000) => {
...
const registerUser = async (payload) => {
    ...
    if (role !== UserRole.USER && role !== UserRole.DONOR) {
        throw new AppError(status.BAD_REQUEST, "Please choose a valid role: USER or DONOR.");
    }
    ...
    if (existingUser) {
        throw new AppError(status.CONFLICT, "An account with this email already exists. Please sign in instead.");
    }
    let signUpResult;
    try {
        ...
        throw new AppError(status.BAD_REQUEST, "We could not create your account right now. Please check your details and try again.");
    }
    if (!signUpResult?.user) {
        throw new AppError(status.INTERNAL_SERVER_ERROR, "Account created partially, but we could not complete sign up. Please try again.");
    }
    try {
        let donor = null;
        ...
        throw new AppError(status.INTERNAL_SERVER_ERROR, "Your account could not be completed. Please try signing up again.");
    }
};
const loginUser = async (payload) => {
    ...
    try {
        ...
        throw new AppError(status.UNAUTHORIZED, "The email or password you entered is not correct.");
    }
    if (!loginResult?.user) {
        throw new AppError(status.UNAUTHORIZED, "The email or password you entered is not correct.");
    }
    assertAccountUsable(loginResult.user.status, loginResult.user.isDeleted);
    const tokenPayload = {
...
const getNewToken = async (refreshToken, sessionToken) => {
    ...
    if (!session) {
        throw new AppError(status.UNAUTHORIZED, "Your session could not be found. Please sign in again.");
    }
    ...
    if (!verifiedRefreshToken.success) {
        throw new AppError(status.UNAUTHORIZED, "Your refresh token is invalid or expired. Please sign in again.");
    }
    const decoded = verifiedRefreshToken.data;
    if (decoded.userId !== session.user.id) {
        throw new AppError(status.UNAUTHORIZED, "Your session does not match this refresh token. Please sign in again.");
    }
    const tokenPayload = {
    ...
    const updatedSession = await prisma.session.update({
        ...
        data: {
            expiresAt: new Date(Date.now() + envVar.BETTER_AUTH_SESSION_TOKEN_EXPIRES_IN * 1000),
            updatedAt: new Date(),
        },
    });
    return {
...
const changePassword = async (payload, sessionToken) => {
    ...
    if (!session?.user) {
        throw new AppError(status.UNAUTHORIZED, "Your session is not valid. Please sign in again.");
    }
    ...
    if (account?.providerId && !["email", "credential"].includes(account.providerId)) {
        throw new AppError(status.BAD_REQUEST, "Password change is not available for social sign-in accounts.");
    }
    let result;
    try {
        ...
        throw new AppError(status.BAD_REQUEST, "Could not change password. Please verify your current password and try again.");
    }
    const tokenPayload = {
        userId: session.user.id,
...
const verifyEmail = async (otp, email) => {
    ...
    try {
        ...
        throw new AppError(status.BAD_REQUEST, "The verification code is invalid or expired. Please request a new one.");
    }
...
const resendVerificationOtp = async (email) => {
    ...
    if (!user) {
        throw new AppError(status.NOT_FOUND, "No account was found with this email address.");
    }
    if (user.emailVerified) {
        throw new AppError(status.BAD_REQUEST, "This email is already verified. You can sign in now.");
    }
...
const forgetPassword = async (email) => {
    ...
    if (!user.emailVerified) {
        throw new AppError(status.BAD_REQUEST, "Please verify your email before requesting a password reset.");
    }
    assertAccountUsable(user.status, user.isDeleted);
    const account = await prisma.account.findFirst({
        where: {
    ...
    if (account?.providerId && !["email", "credential"].includes(account.providerId)) {
        throw new AppError(status.BAD_REQUEST, "Password reset is not available for social sign-in accounts.");
    }
...
const resetPassword = async (payload) => {
    ...
    if (!user.emailVerified) {
        throw new AppError(status.BAD_REQUEST, "Please verify your email before resetting your password.");
    }
    assertAccountUsable(user.status, user.isDeleted);
    const account = await prisma.account.findFirst({
        where: {
    ...
    try {
        ...
        throw new AppError(status.BAD_REQUEST, "The OTP is invalid or expired. Please request a new password reset code.");
    }
...
const updateProfileImage = async (userId, file) => {
    ...
    const uploadedImage = await uploadImageWithTimeout(file, 15000).catch((error) => {
        throw new AppError(status.BAD_GATEWAY, error instanceof Error ? error.message : "Failed to upload profile image.");
    });
