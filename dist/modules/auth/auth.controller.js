...
const getNewToken = catchAsync(async (req, res) => {
    ...
    if (!refreshToken) {
        throw new AppError(status.UNAUTHORIZED, "Refresh token not found. Please sign in again.");
    }
    if (!sessionToken) {
        throw new AppError(status.UNAUTHORIZED, "Session token not found. Please sign in again.");
    }
