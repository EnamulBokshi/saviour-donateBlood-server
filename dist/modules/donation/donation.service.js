...
const assertCanManageDonation = async (donation, currentUser) => {
    if (isAdminOrSuperAdmin(currentUser.role)) {
        return;
    }
    if (currentUser.role === UserRole.DONOR) {
...
const getDonations = async (query, currentUser) => {
    const where = await getAccessWhere(currentUser);
    const queryBuilder = new QueryBuilder(prisma.donation, query, {
...
const getDonationById = async (id, currentUser) => {
...
const createDonation = async (payload, currentUser) => {
    const donor = await prisma.donor.findFirst({
        where: {
            id: payload.donorId,
            isDeleted: false,
...
const updateDonation = async (id, payload, currentUser) => {
    const donation = await prisma.donation.findFirst({
        where: {
            id,
            isDeleted: false,
...
const softDeleteDonation = async (id, currentUser) => {
    const donation = await prisma.donation.findFirst({
        where: {
            id,
            isDeleted: false,
